...
class MyProblem(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        u = Vars[:, [0]]#μmax
        K = Vars[:, [1]]#Ks
        real_data_raw = pd.read_csv('./data/ANAMMOX3.csv',
                                    header=0, index_col=None,
                                    usecols=['Mins', 'NO2--N'],  # 这里要使用两列数据
                                    encoding='utf-8',nrows=11)
        real_data = real_data_raw[1:]
        ...
        for index,predict in enumerate(predict_data):# 因为没有使用real_data最后一个数据，所以预测出的predict_data[-1]==C_s_list[-1]
            score = score + ((C_s_list[index + 1] - predict)/C_s_list[index + 1]) ** 2  # SSE
        score=score/len(C_s_list)
