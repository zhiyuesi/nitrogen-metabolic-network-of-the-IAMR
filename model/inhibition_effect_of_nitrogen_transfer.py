...
# @File    : inhibition_effect_of_nitrogen_transfer.py
# @version  : 20200814.0
...
def COD_FRAC_FOR_DE_NITRITE(DO,VSS,nitrite,nitrate,IsMicroaerobic=True,
    ...
    return  de_nitrite_rate * DE_NITRITE_COD / (cod_oxidation_rate*IsMicroaerobic +
                                                                  ...
                                                                  nitrate_respiration_rate * NITRATE_RESPIRATION_COD +de_nitrite_rate * DE_NITRITE_COD)
...
def diluent(substance, diluent_radio=8.0 * 60 * 60):
    '''
    稀释速率。反应器是完全混合的，所以进水进入反应器立刻被稀释：
        Qin(L/h)*HRT(h)=V(L)
        dCin/dt=QCin/V
        联立：dCin/dt=Cin/HRT(mg·L-1·h-1)
        HRT=8 h，且我们模拟的最小步长是 1sec,所以有
...
COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR=COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR) / DE_NITRATE_COD
        ...
        nitrate2nitrite = COD_current * COD_FRAC_FOR_DE_NITRATE_RESPIRATION(DO,VSS,nitrite,nitrate,IsMicroaerobic=True,COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR=COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR) / NITRATE_RESPIRATION_COD
        ...
        DE_nitrite = COD_current * COD_FRAC_FOR_DE_NITRITE(DO,VSS,nitrite,nitrate,IsMicroaerobic=True,COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR=COD_ANAEROBIC_DIGESTION_INHIBITION_FACTOR) / DE_NITRITE_COD
...
def simulate(ammonia_input, COD_input,
    ...
    while True:
        ...
        if not LastPeriod_Mean:
            ...
            # 当仿真时间未达14小时，且存在一个指标误差大于0.001，则再次计算
...
def simulate_params(experimental_datas:pd.DataFrame,**kwargs):
    '''
    kwargs里的变量必须是list类型，且长度等于experimental_datas.shape[0]。
    :param experimental_datas:
    :param kwargs:simulate（）其他的关键字参数
    :return:
    '''
...
if __name__ == '__main__':
    ...
    # 结果保存路径
    parser.add_argument('--result_path', '-p',type=str,default="./inhibition", help='problem class')
    ...
    #DO不需要初始化，但是为了对齐实验数据，需要还是生成跟simulate_datas一样长的数据
    ...
    combined_inhibition_factor_list=[(x,y) for x in anammox_inhibition_factor_list for y in cod_anaerobic_digestion_inhibition_factor_list]
    ...
    if args.append:
        ...
        anammox_inhibition_factor_str,cod_anaerobic_digestion_inhibition_factor_str=re.findall('\d+.?\d+',latest_file_created)
        begin_pos=combined_inhibition_factor_list.index((float(anammox_inhibition_factor_str),
                                                float(cod_anaerobic_digestion_inhibition_factor_str)))
        # 将旧的已经仿真的内容去除，然后生成新的仿真参数列表
        combined_inhibition_factor_list=combined_inhibition_factor_list[begin_pos:]
        # 将其变成元组的形式，并找到其在列表中的位置。
        # 因为如果程序卡死，最后一个文件可能不完整。所以这个file要删除
        # 遍历删除文件夹下面的文件，最后删除文件夹
    ...
    #变成tqdm对象，方便输出进度条
    combined_inhibition_factor_list = tqdm(combined_inhibition_factor_list)
    for anammox_inhibition_factor,cod_anaerobic_digestion_inhibition_factor in combined_inhibition_factor_list:
        combined_inhibition_factor_list.set_description(f"Processing anammox_inhibition_factor:{anammox_inhibition_factor},cod_anaerobic_digestion_inhibition_factor：{cod_anaerobic_digestion_inhibition_factor}")

        simulate_root_path = os.path.join(result_path,f"anammox{anammox_inhibition_factor}&COD{cod_anaerobic_digestion_inhibition_factor}")
